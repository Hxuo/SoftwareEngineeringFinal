...
function updateTimeSlots() {
    ...
    fetch(`../Backend-User/fetch_timeslots.php?date=${selectedDate}&duration=${slotDuration}`)
        .then(response => response.json())
...
function Loginprocess() {
  ...
  function googleLogin(name, email) {
    ...
    fetch('../Backend-User/google-login.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
function clearCreateAccountFields() {
  ...
  function checkEmailExists(email, errorElement) {
    fetch('../Backend-User/CheckEmail.php?email=' + encodeURIComponent(email))
        .then(response => response.json())
        .then(data => {
            if (data.exists) {
...
function editAccount() {
  ...
  function createEditprocess() {
    ...
    fetch('../Backend-User/edit_account.php', {
        method: 'POST',
        headers: {
