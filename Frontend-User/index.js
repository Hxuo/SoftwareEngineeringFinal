...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Clone cards for seamless rotation
  const cards = [...document.querySelectorAll(".card")];
  cards.forEach((card) => {
    const cloneStart = card.cloneNode(true);
    const cloneEnd = card.cloneNode(true);
    cardContainer.appendChild(cloneEnd);
    cardContainer.insertBefore(cloneStart, cardContainer.firstElementChild);
  });
  ...
  const scrollSmoothly = (direction) => {
    ...
    setTimeout(() => {
      const maxScrollLeft = (totalCards * 2) * cardWidth;
    ...
    }, 300); // Matches the "smooth" scroll duration
  };
...
function Loginprocess() {
  ...
  fetch('../Backend-User/Login-process.php', {
      method: 'POST',
...
function createprocess() {
  ...
  fetch('../Backend-User/Registration.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
function validateEmail() {
  ...
  if (!createemail.includes('@')) {
      ...
      return false; // Don't proceed with existence check
  }
...
function staffprocess() {
  ...
  fetch('../Backend-User/validateStaffPassword.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
...
function fetchUserAppointments() {
    ...
    fetch(`../Backend-User/fetch_user_appointments.php?email=${userEmail}`)
        .then(response => response.json())
...
function fetchUserAppointmentHistory() {
    const userEmail = '<?php echo $email; ?>'; // Use the session email from PHP
    fetch(`../Backend-User/fetch_user_appointment_history.php?email=${userEmail}`)
        .then(response => response.json())
...
function loadReschedCalendar(type) {
  ...
  fetch(`../Backend-User/fetch_calendar.php?month=${reschedSelectedMonth}&year=${reschedSelectedYear}`)
      .then(response => response.json())
...
function submitReschedule(event) {
  ...
  fetch("../Backend-User/update_reschedule.php", {
      method: "POST",
      body: formData
  }).then(response => response.json())
    .then(data => {
...
function processRefund() {
    ...
    fetch('../Backend-User/process_refund.php', {
        method: 'POST',
        headers: {
